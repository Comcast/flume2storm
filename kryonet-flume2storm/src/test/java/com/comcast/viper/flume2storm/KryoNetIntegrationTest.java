...
import com.comcast.viper.flume2storm.receptor.KryoNetEventReceptor;
import com.comcast.viper.flume2storm.receptor.KryoNetEventReceptorStats;
import com.comcast.viper.flume2storm.sender.KryoNetEventSender;
import com.comcast.viper.flume2storm.sender.KryoNetEventSenderStats;
import com.comcast.viper.flume2storm.sender.KryoNetEventSenderStatus;
...
public class KryoNetIntegrationTest {
  ...
  protected KryoNetEventSenderStats senderStats;
  protected KryoNetEventReceptorStats receptorStats;

  @BeforeClass
  public static void init() {
    connectionParameters = new KryoNetConnectionParameters();
    connectionParameters.setObjectBufferSize(512);
    connectionParameters.setWriteBufferSize(5120);
    connectionParameters.setServerAddress("localhost");
    connectionParameters.setServerPort(21345);
  ...
  public void setup() throws Exception {
    ...
    sender.start();
    senderStats = new KryoNetEventSenderStats();
    receptorStats = new KryoNetEventReceptorStats();
    KryoNetTestUtil.checkSenderStatus(sender, senderStats.setStatus(KryoNetEventSenderStatus.STARTED));
  }
  ...
  public void testServerRestart() throws Exception {
    ...
    KryoNetTestUtil.waitSenderShutdown(sender, TEST_TO);
    Thread.sleep(4000);
    KryoNetTestUtil.checkSenderStatus(sender, senderStats.decrClients().setStatus(KryoNetEventSenderStatus.STOPPED));
    KryoNetTestUtil.checkReceptorStatus(receptor, receptorStats.setDisconnected());
    ...
    KryoNetTestUtil.checkSenderStatus(sender, senderStats.reset().setStatus(KryoNetEventSenderStatus.STARTED)
        .incrClients().incrEventsIn(5).incrEventsOut(5));
  ...
  public void testBadConnection() throws Exception {
    ...
    tcpForwarderBuilder.setListenAddress(proxyHost).setInputPort(proxyPort)
        .setOutputServer(connectionParameters.getServerAddress()).setOutputPort(connectionParameters.getServerPort());
