...
public class KryoNetSimpleRealtimeStrategy implements KryoNetEventSenderStrategy, Serializable {
  ...
  /**
   * @param event
   *          The {@link F2SEvent} to send
   * @return True if the event was sent successfully, false otherwise
   */
  private boolean sendF2SEvent(Connection client, F2SEvent event) {
    if (client.sendTCP(event) == 0) {
      LOG.debug("Failed to send {} to {}", event, client);
      knEventSender.getStats().incrEventsFailed();
      return false;
    }
    knEventSender.getStats().incrEventsOut();
    LOG.trace("Sent {} to {}", event, client);
    return true;
  }

  /**
   * @see com.comcast.viper.flume2storm.sender.KryoNetEventSenderStrategy#send(java.util.List)
   */
  @Override
