...
public class ZkClient {
	 ...
	 * 
	 * @param listener
	 *            The {@link ZkClientListener}. Can have only one of these, but
	 *            it may be null.
	 */
	 ...
	 * established). So in the main loop thread (i.e. ConnectionManager), we
	...
	protected class ConnectionManager extends Thread implements ZkClientStateListener {
		...
		private final AtomicReference<ZkSession> currentSession;
		...
		private final void clearZkSession() {
		...
		/**
		 * This is the connection manager thread, where we mostly wait (so that
		 * we don't do it in the event thread - i.e. in the connection watcher
		 * process method)
		 * 
				...
				try {
					...
					case CONNECTING:
						if (started.get()) {
							...
							/*
							 * Programming note: we didn't initialize, so we
							 * shouldn't call the clean up
							 */
							exitMainLoop = true;
						}
						break;
					...
					case CONNECTED:
						if (started.get()) {
							...
							while (zookeeper.get().getSessionId() == 0 && System.currentTimeMillis() - t0 < config.get().getConnectionTimeout()
									&& started.get()) {
								Thread.sleep(100);
							}
		...
		protected boolean connect() {
			...
			try {
				if (session.isSet()) {
					...
					zookeeper.set(new ZooKeeper(configuration.getConnectionStr(), configuration.getSessionTimeout(), connectionWatcher, session.getSessionId(),
							session.getSessionPwd()));
				} else {
					LOG.debug("Connecting to ZK server using new session...");
					zookeeper.set(new ZooKeeper(configuration.getConnectionStr(), configuration.getSessionTimeout(), connectionWatcher));
				}
				return true;
		...
		protected final Watcher connectionWatcher = new Watcher() {
			...
			public void process(final WatchedEvent event) {
				...
				case SyncConnected:
					...
					case CONNECTING:
						/*
						 * Regular connection: our (ZkClient) connection attempt
						 * has been successful
						 */
				...
				case Expired:
					/*
					 * The session expired - this may happen when something went
					 * wrong in the comm between the client and the server (but
					 * both were still running). Zookeeper will attempt to
					 * reconnect
					 */
