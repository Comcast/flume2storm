...
public class ZkOperationTest extends ZkClientTestUtils {
	...
	public void testNodeManagement() throws Exception {
		...
		LOG.info("### Test step: Creating ZK server...");
		...
		Assert.assertEquals(node1path,
				zkOp.createNode(new ZkNodeCreationArg().setData(node1data).setCreateMode(CreateMode.PERSISTENT).setCreateHierarchy(false)));
		...
		Assert.assertEquals(deepPath,
				zkOp.path(deepPath).createNode(new ZkNodeCreationArg().setData(deepData).setCreateHierarchy(true).setCreateMode(CreateMode.EPHEMERAL)));
		...
		// And again:
		Assert.assertEquals(deepPath, zkOp.path(deepPath).createNode(new ZkNodeCreationArg().setCreateHierarchy(true).setCreateMode(CreateMode.EPHEMERAL)));
		Assert.assertTrue(new ZkOperation(zkClient, "/me").nodeExists());
		Assert.assertArrayEquals(new byte[0], new ZkOperation(zkClient, "/me").getData());
		Assert.assertTrue(new ZkOperation(zkClient, "/me/and").nodeExists());
		Assert.assertArrayEquals(new byte[0], new ZkOperation(zkClient, "/me/and").getData());
